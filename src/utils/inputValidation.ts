...
export const validateArtworkTitle = (
  title: string
...
export const validateArtworkDescription = (
  description: string
): { isValid: boolean; error?: string } => {
...
export const validatePlatformLink = (
  url: string
): { isValid: boolean; error?: string } => {
  ...
  try {
    ...
    if (!allowedDomains.includes(urlObj.hostname)) {
      return {
        isValid: false,
        error: "Platform link must be from a supported platform",
      };
    }
...
export const validateImageFile = (
  ...
  if (file.size > maxSize) {
    return { isValid: false, error: "Image must be smaller than 50MB" };
  }
